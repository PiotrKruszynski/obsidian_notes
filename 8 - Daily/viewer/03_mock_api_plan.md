...
- Do not modify `api/`.
- Do not modify `openapi.yaml`; record gaps for phase 04.
...
- [ ] (YYYY-MM-DD HH:MMZ) Compare service names and payload shapes with `openapi.yaml` at a high level.
...
- Services may drift from `openapi.yaml`; phase 04 must reconcile this.
