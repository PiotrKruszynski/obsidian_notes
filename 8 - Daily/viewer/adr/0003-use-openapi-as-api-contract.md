...
The project already includes `openapi.yaml`. This contract must bridge the domain model, frontend service layer and backend implementation.

## Decision

Use `openapi.yaml` as the API contract between the PWA and the FastAPI backend.

The project should follow these rules:

- `openapi.yaml` is the source of truth for backend endpoint paths, request schemas, response schemas and error shapes after the OpenAPI alignment phase;
- backend agents must implement endpoints defined in `openapi.yaml`, not invent additional MVP endpoints during implementation;
- frontend integration must call the backend through service functions aligned with `openapi.yaml`;
