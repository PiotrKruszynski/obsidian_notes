...
The backend lives in `api/` and is expected to implement the aligned `openapi.yaml` contract. The frontend lives in `pwa/` and will integrate with the backend after the mock service layer is completed. QA must be able to reset the database to a deterministic state before automated tests.
