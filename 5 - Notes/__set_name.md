jeżeli potrzebujesz __set_name to powinieneś używać deskryptora z __set_name zamist metaklasy

```python
class Positive: # obiekt przechwytuje odczyt, zapis i usuwanie atrybutów w innej klasie  
    def __init__(self):  
    ...
    # jeżeli chciałbym wiedzieć, za co odpowiada błąd # pluto.radius_metres = -1 # ValueError: Value -1 is not positive UŻYWAM METAKLASY    def __set_name__(self, owner, name):  
        self._name = name  
    ...
    def __delete__(self, instance):  
        raise AttributeError(f"cant delete attribute {self._name}")  
